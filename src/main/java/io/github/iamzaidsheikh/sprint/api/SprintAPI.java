...
public class SprintAPI {
    ...
    public ResponseEntity<UserProfile> registerUser(@RequestBody @Valid UserDTO data) {
        var profile = us.registerUser(data);
        return ResponseEntity
                .created(ServletUriComponentsBuilder.fromCurrentContextPath().path("api/v1/profile/{username}")
                        .buildAndExpand(profile.getUsername()).toUri())
                .body(profile);
    }
    ...
    @PostMapping("/goals")
    public ResponseEntity<String> createGoal(@RequestBody @Valid GoalDTO data, Principal principal) {
        var goalId = gs.createGoal(principal.getName(), data);
        var uri = ServletUriComponentsBuilder.fromCurrentContextPath().path("api/v1/goals/{goalId}")
    ...
    public ResponseEntity<String> extendDeadline(@PathVariable String goalId, @RequestBody @Valid ExtendDTO data,
            Principal principal) {
        var uri = ServletUriComponentsBuilder.fromCurrentContextPath().path("api/v1/goals/{goalId}")
                .buildAndExpand(goalId)
                .toUri();
    ...
    public ResponseEntity<String> submitTask(@PathVariable String goalId, @PathVariable String taskId,
            @RequestBody @Valid SubmitTaskDTO submission,
            Principal principal) {
        var uri = ServletUriComponentsBuilder.fromCurrentContextPath().path("api/v1/goals/{goalId}")
                .buildAndExpand(goalId)
                .toUri();
