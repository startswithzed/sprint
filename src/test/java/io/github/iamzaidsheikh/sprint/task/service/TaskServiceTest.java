...
public class TaskServiceTest {
  ...
  void testCanCreateTask() {
    ...
    var goal = new Goal(
        ...
        "Test Title",
        "Test Desc");
    var data = new TaskDTO("Test Task Desc", Instant.now().minusSeconds(60 * 60 * 4));
    // when
  ...
  void testCouldNotCreateTaskBecauseUserIsNotAuthorOrMentor() {
    ...
    var data = new TaskDTO("Test Task Desc", Instant.now().minusSeconds(60 * 60 * 24 * 6));
    var user = "testUser";
    // when
  ...
  void testCouldNotCreateTaskBecauseTaskIsAfterGoalDeadline() {
    ...
    var goal = new Goal(
        ...
        "Test Title",
        "Test Desc");
    var data = new TaskDTO("Test Task Desc", Instant.now().plusSeconds(1));
    // when
    when(gr.findById(id)).thenReturn(Optional.of(goal));
    // then
    AssertionsForClassTypes.assertThatThrownBy(() -> underTest.createTask(id, user, data))
  ...
  void testCouldNotFindTask() {
    ...
    AssertionsForClassTypes
        .assertThatThrownBy(() -> underTest.submitTask(goalId, taskId, user, new SubmitTaskDTO("comment", "link")))
